...
OUTPUT_FILE="${2:-rk3528-disable-vpu-power-domain.dtsi}"

if [ -z "$INPUT_FILE" ]; then
    echo "Usage: $0 <input_dtsi> [output_dtsi]"
    echo "Example: $0 rk3528.dtsi rk3528-disable-vpu-power-domain.dtsi"
    exit 1
fi
...
grep -n "power-domains\s*=\s*<&power RK3528_PD_VPU>" "$INPUT_FILE" | while read -r line; do
...
/*
 * Device tree override to remove RK3528_PD_VPU power domain (so it will always on)
 * RK3528_PD_VPU power domain affects gpio1&3
