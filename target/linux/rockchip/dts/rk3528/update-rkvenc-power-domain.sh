...
INPUT_FILE="${1:-arch/arm64/boot/dts/rockchip/rk3528.dtsi}"
OUTPUT_FILE="${2:-rk3528-disable-rkvenc-power-domain.dtsi}"

if [ -z "$INPUT_FILE" ]; then
    echo "Usage: $0 <input_dtsi> [output_dtsi]"
    echo "Example: $0 rk3528.dtsi rk3528-disable-rkvenc-power-domain.dtsi"
    exit 1
fi
...
grep -n "power-domains\s*=\s*<&power RK3528_PD_RKVENC>" "$INPUT_FILE" | while read -r line; do
...
/*
 * Device tree override to remove RK3528_PD_RKVENC power domain (so it will always on)
 * RK3528_PD_RKVENC power domain affects i2c0 and i2c1, which may used by PMIC
