...
dnsmasq_start()
{
	...
	[ $dns_redirect = 1 ] && {
		iptables -t nat -w -A PREROUTING -m comment --comment "Dnsmasq" -p udp --dport 53 -j REDIRECT --to-ports $dns_port
		ip6tables -t nat -w -A PREROUTING -m comment --comment "Dnsmasq" -p udp --dport 53 -j REDIRECT --to-ports $dns_port 2>/dev/null
	}
}
...
iptables_clear()
{
	for ipt in "iptables -t nat -w" "ip6tables -t nat -w"; do
		for i in $(seq 1 $($ipt -nL PREROUTING | grep -c "Dnsmasq")); do
			local index=$($ipt --line-number -nL PREROUTING | grep "Dnsmasq" | head -1 | awk '{print $1}')
			$ipt -D PREROUTING $index 2>/dev/null
		done
	done
}
