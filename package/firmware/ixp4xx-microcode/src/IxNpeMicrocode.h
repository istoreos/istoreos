...
int main(int argc, char *argv[])
{
		...
		image = (struct dl_image *)(arrayptr), cnt++)
	{
		...
		*(unsigned*)field = to_be32(image->id);
		...
		snprintf(filename, sizeof(filename), "NPE-%c.%08x", (field[0] & 0xf) + 'A',
			image->id);
		if (image->id == 0x00090000)
			snprintf(slnk, sizeof(slnk), "NPE-%c-HSS", (field[0] & 0xf) + 'A');
		else
			snprintf(slnk, sizeof(slnk), "NPE-%c", (field[0] & 0xf) + 'A');
